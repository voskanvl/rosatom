...
// })
// //---
// --- для всех кнопок содержащих "Заказать услугу" вызывать модальное окно
const KEYWORDS_TO_MODAL_WINDOW = ["Заказать услугу", "Заказать размещение"]
const buttonsOrder = document.querySelectorAll<HTMLButtonElement>("button")
buttonsOrder &&
    buttonsOrder.length &&
    [...buttonsOrder]
        .filter(e => KEYWORDS_TO_MODAL_WINDOW.some(k => e.innerHTML.includes(k)))
        .forEach(e => e.addEventListener("click", () => modal(feedback())))
